...
public class MobSpawnOnce extends BukkitRunnable {
    ...
    public MobSpawnOnce(World world,JavaPlugin plugin,Player player ,ProtectedRegion region, int[][] mbcord, String[] mobs, int sx, int sy, int sz, int level) {
        this.plugin = plugin;
        this.region = region;
    ...
    public void run() {
        ...
        if (this.region.getFlag(HAS_AC_QUESTS_FLAG)){
            ...
            //player.chat("boo");
            for(int i = 0; i < MobCoords.length; i++){
                int[] te = MobCoords[i];
                ...
                int tez = te[2] + sz;
                int tet = te[3];
                Bukkit.getServer().dispatchCommand(getConsoleSender(),"mm mobs spawn -s " + mobs[tet] +":"+ this.level + " 1 world," + tex + "," + tey + "," + tez);
